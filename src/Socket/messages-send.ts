...
export const makeMessagesSocket = (config: SocketConfig) => {
	...
	return {
		...
		sendMessage: async (jid: string, content: AnyMessageContent, options: MiscMessageGenerationOptions = {}) => {
			...
			// ===== PATCH: support groupStatusMessage =====
if ((content as any)?.groupStatusMessage) {
  const gsm = (content as any).groupStatusMessage

  // target status
  const statusJid = 'status@broadcast'

  // kalau user ngirim dari grup, kita simpan grupnya sebagai participant
  // biar "status grup" kebaca benar
  const participant = jid

  // normalisasi: ubah ke format message yg bisa di-relay
  // NOTE: kita kirim via sendMessage ke status@broadcast supaya pipeline media tetap kepakai
  // statusJidList: minimal 1 jid (biar WA nganggap ada audience)
  const newOptions: any = {
    ...(options || {}),
    statusJidList: (options as any)?.statusJidList || [jid],
    additionalAttributes: {
      ...(options as any)?.additionalAttributes,
      participant
    }
  }

  // mapping gsm -> content biasa yang didukung sendMessage
  if (gsm.text) {
    return await sendMessage(statusJid, { text: gsm.text }, newOptions)
  }

  if (gsm.image) {
    return await sendMessage(statusJid, { image: gsm.image, caption: gsm.caption || '' }, newOptions)
  }

  if (gsm.video) {
    return await sendMessage(statusJid, { video: gsm.video, caption: gsm.caption || '' }, newOptions)
  }

  if (gsm.audio) {
    return await sendMessage(statusJid, { audio: gsm.audio }, newOptions)
  }

  throw new Error('groupStatusMessage: unsupported payload')
}
// ===== END PATCH =====
			if (
